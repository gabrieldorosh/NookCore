...
public final class ChestShopRentalGate implements Listener {
    ...
    public void create(PreShopCreationEvent event) {
        ...
        }catch(IllegalArgumentException ex){event.setCancelled(true);event.getPlayer().sendMessage(NookUi.problem("NookPlots",ex.getMessage()));}
        catch(Exception ex){event.setCancelled(true);failure.accept(ex);event.getPlayer().sendMessage(NookUi.message("NookPlots","Shop creation is paused; contact staff."));}
    }
    ...
    public void trade(PreTransactionEvent event) {
        try {
            ...
            event.setCancelled(true);event.getClient().sendMessage(NookUi.problem("NookPlots",ex.getMessage()));
        }catch(Exception ex) {
            event.setCancelled(true);failure.accept(ex);event.getClient().sendMessage(NookUi.message("NookPlots","Shop payments are paused; contact staff."));
        }
    }
}
