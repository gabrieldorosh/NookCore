...
final class ShopSetup implements Listener,CommandExecutor,TabCompleter {
    ...
    @Override public boolean onCommand(CommandSender sender,Command command,String label,String[] args){
        ...
        try{
            if(args.length==0 || args[0].equalsIgnoreCase("help")){
                NookUi.help(sender,"NookShops · Setup preview","/nookshops create — hold your sale item and look at your stock chest","/nookshops preview — reopen your draft","/nookshops price <nooks> — set the price per bundle","/nookshops payment <item> <quantity> — use plain items as payment","/nookshops cancel — discard your draft");
            ...
            if(sub.equals("create") && args.length==1){
                ...
                if(chest==null || chest.getType()!=Material.CHEST && chest.getType()!=Material.TRAPPED_CHEST)throw new IllegalArgumentException("Look at a chest within five blocks while holding the item you want to sell.");
            ...
            if(sub.equals("price") && args.length==2)terms=terms.nooks(Money.parse(args[1]));
            else if(sub.equals("payment") && args.length==3){
                ...
                terms=terms.item(material.name(),Integer.parseInt(args[2]));
    ...
    private Draft current(Player player){
        ...
        if(draft==null || System.currentTimeMillis()>=draft.expires()){drafts.remove(player.getUniqueId());throw new IllegalArgumentException("No current draft. Hold your sale item, look at a chest and run /nookshops create.");}
        if(!player.getWorld().equals(draft.chest().getWorld()) || !draft.chest().getWorld().isChunkLoaded(draft.chest().getX()>>4,draft.chest().getZ()>>4))throw new IllegalArgumentException("Return near your stock chest before continuing setup.");
        if(draft.chest().getType()!=Material.CHEST && draft.chest().getType()!=Material.TRAPPED_CHEST)throw new IllegalArgumentException("The selected chest is no longer there. Start a new draft.");
        validate.accept(player,draft.chest());return draft;
    }
    private ItemStack icon(Material type,String title,String... lines){
        var item=new ItemStack(type);var meta=item.getItemMeta();meta.displayName(Component.text(title,NookUi.ACCENT));
