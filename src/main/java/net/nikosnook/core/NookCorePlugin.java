...
public final class NookCorePlugin extends JavaPlugin implements Listener, CommandExecutor, TabCompleter {
    ...
    private void joinPlayer(Player p)throws SQLException {
        ...
        if(store.join(p.getUniqueId(),p.getName(),System.currentTimeMillis(),bonus))p.sendMessage("Welcome to Niko's Nook! Your starting balance is "+Money.format(bonus)+".");
        else p.sendMessage("Your balance: "+Money.format(store.account(p.getUniqueId()).orElseThrow().cents()));
    ...
    private void plotNotice(Player player,NookStore.Plot plot)throws SQLException {
        ...
        if(plot.state().equals("GRACE"))player.sendMessage("Plot "+plot.id()+" is overdue and sales are closed. The original renter can /nookplots reopen "+plot.id()+" before "+java.time.Instant.ofEpochMilli(plot.paidUntil()+NookStore.WEEK)+". Only the remaining part of the week is charged.");
        else if(plot.state().equals("RECLAIM"))player.sendMessage("Plot "+plot.id()+" is awaiting staff reclamation. Sales are closed; contact staff about collecting your belongings. Nothing is automatically deleted.");
    }
    ...
    public void teleport(PlayerTeleportEvent e){
        ...
        if((env==World.Environment.NETHER && getConfig().getBoolean("travel.lock-nether",true)) || (env==World.Environment.THE_END && getConfig().getBoolean("travel.lock-end",true))){e.setCancelled(true);e.getPlayer().sendMessage("That dimension has not opened yet.");return;}
        if(getConfig().getBoolean("travel.block-command-teleports",true) && (e.getCause()==PlayerTeleportEvent.TeleportCause.COMMAND || e.getCause()==PlayerTeleportEvent.TeleportCause.PLUGIN)){
            e.setCancelled(true);e.getPlayer().sendMessage("Use the world's roads, rails and vanilla travel methods to get around.");
        }
    }
    ...
    @Override public boolean onCommand(CommandSender sender,Command command,String label,String[] args){
        if(!healthy){sender.sendMessage("The economy is paused. Please contact an admin.");return true;}
        try{
            long now=System.currentTimeMillis();
            if(command.getName().equals("nookadmin")){
                if(!sender.hasPermission("nookcore.admin")){sender.sendMessage("You don't have permission.");return true;}
                if(args.length==3 && args[0].equalsIgnoreCase("plotdefine")){
                    store.definePlot(args[1],Money.parse(args[2]));sender.sendMessage("Plot record exists: "+args[1]+", "+Money.format(store.plot(args[1]).weekly())+"/week. Configure its WorldGuard mapping before rentals are enabled.");return true;
                }
                if(args.length>=4 && args[0].equalsIgnoreCase("plotabsence")){
                    ...
                    sender.sendMessage("Absence exception recorded. Rent still requires funds; this does not reopen a closed shop or waive rent.");return true;
                }
                if(args.length>=3 && args[0].equalsIgnoreCase("plotclear")){
                    ...
                    store.confirmCleared(args[1],sender.getName(),String.join(" ",Arrays.copyOfRange(args,2,args.length)),now);reconcilePlots();sender.sendMessage("Recorded archive reference and released plot. This command did not pack items or clear world blocks.");return true;
                }
                if(args.length==1 && args[0].equalsIgnoreCase("backup")){
                    Path folder=getDataFolder().toPath().resolve("backups");Files.createDirectories(folder);Path file=folder.resolve("nooks-"+now+".db");store.backup(file);sender.sendMessage("Consistent economy backup saved: "+file.getFileName());return true;
                }
                if(args.length==2 && args[0].equalsIgnoreCase("awards") && Set.of("on","off").contains(args[1].toLowerCase(Locale.ROOT))){
                    awardsEnabled=args[1].equalsIgnoreCase("on");getConfig().set("advancement-rewards-enabled",awardsEnabled);saveConfig();sender.sendMessage("Advancement rewards "+(awardsEnabled?"enabled":"disabled")+". Disabled-period advancements are not backfilled.");return true;
                }
            ...
            if(!(sender instanceof Player p)){sender.sendMessage("Use /nookadmin balance <player> from console.");return true;}
            if(args.length==0){sender.sendMessage("Your balance: "+Money.format(store.account(p.getUniqueId()).orElseThrow().cents()));return true;}
            if(args.length==1 && args[0].equalsIgnoreCase("history")){for(var row:store.history(p.getUniqueId()))sender.sendMessage("#"+row.id()+" "+Money.format(row.delta())+" · "+row.kind()+" · "+row.reference());return true;}
        ...
        }catch(IllegalArgumentException e){sender.sendMessage(e.getMessage());return true;}
        catch(Exception e){fail(e);sender.sendMessage("The operation could not be confirmed. Contact staff before retrying.");return true;}
    }
