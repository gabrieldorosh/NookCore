...
class PlotCommandsTest {
    ...
    @Test void rentalLockLeavesBalanceMembershipAndProtectionUnchanged()throws Exception {
        var c=commands(true,true,()->{},reconciles::incrementAndGet);c.rentalsOpenAt(System.currentTimeMillis()+86400000L);
        long balance=store.account(id).orElseThrow().cents();rent(c);
        assertEquals(balance,store.account(id).orElseThrow().cents());assertEquals("AVAILABLE",store.plot("one").state());assertEquals(0,reconciles.get());
        c.rentalsOpenAt(0);rent(c);assertEquals("OWNER",store.role("one",id));
    }
    @Test void addressSuggestionsHideIdsAndCapacityMessageIsPersonal()throws Exception {
        ...
        assertEquals(List.of("willow-way"),c.onTabComplete(player,null,"plots",new String[]{"rent",""}));
        assertEquals(List.of("willow-way"),c.onTabComplete(player,null,"plots",new String[]{"info",""}));
    ...
    @Test void infoWithoutAddressUsesCurrentPlotAndRejectsOutside(){
        ...
        c.onCommand(viewer,null,"plots",new String[]{"info"});
        assertTrue(lines.stream().anyMatch(s->s.startsWith("Address:")));assertEquals(0,failures.get());
        lines.clear();c.currentPlot(p->null);c.onCommand(viewer,null,"plots",new String[]{"info"});
    ...
    void rent(PlotCommands commands){commands.onCommand(player,null,"plots",new String[]{"rent","one"});}
    @Test void unmappedOwnerCanConfirmAbandonmentAndRentElsewhere()throws Exception {
        ...
        assertTrue(c.onTabComplete(player,null,"plots",new String[]{"abandon",""}).contains("one"));
        c.onCommand(player,null,"plots",new String[]{"abandon","one"});
        assertEquals("OWNER",store.role("one",id));
        c.onCommand(player,null,"plots",new String[]{"abandon","one","confirm"});
    ...
    @Test void addressAliasRentsMappedPlotButCannotBypassMapping()throws Exception {
        ...
        blocked.onCommand(player,null,"plots",new String[]{"rent","willow-way"});
        assertEquals("AVAILABLE",store.plot("one").state());
        var c=commands(true,true,()->{},()->{});
        assertTrue(c.onTabComplete(player,null,"plots",new String[]{"rent","willow"}).contains("willow-way"));
        c.onCommand(player,null,"plots",new String[]{"rent","willow-way"});
    ...
    @Test void malformedRentalCommandsCannotSpendMoneyOrGrantMembership()throws Exception {
        ...
        for(String[] args:new String[][]{{"rent"},{"rent","one","extra"},{"prepay","one","1.5"},{"invite","one","Player"}})
            c.onCommand(player,null,"plots",args);
    ...
    @Test void acceptsNamedInvitationWithoutTypingToken()throws Exception {
        ...
        c.onCommand(player,null,"plots",new String[]{"accept","owner"});assertEquals("STOCK",store.role("other",id));
    }
    ...
    @Test void invitingExistingMemberUpdatesRoleWithoutRemovingMembership()throws Exception {
        ...
        var c=commands(true,true,()->{},()->reconciles.incrementAndGet());c.onCommand(player,null,"plots",new String[]{"invite","one","Member","build"});
    ...
    @Test void invitationCompletionOffersNamesAndNoTokens()throws Exception {
        inviter("Owner","other");var c=commands(true,true,()->{},()->{});
        assertEquals(List.of("Owner"),c.onTabComplete(player,null,"plots",new String[]{"accept","O"}));
    }
    ...
    @Test void consoleCanListWithoutRentingOrSpending()throws Exception {
        ...
        var c=commands(true,true,()->{},()->{});c.onCommand(console,null,"plots",new String[]{"list"});
        assertTrue(messages.stream().anyMatch(v->v.contains("one") && v.contains("Available")),messages.toString());
        c.onCommand(console,null,"plots",new String[]{"rent","one"});
    ...
    @Test void ownerReceivesAcceptAndDeclineOnlyAfterValidInvitation()throws Exception {
        ...
        c.onCommand(player,null,"plots",new String[]{"decline","Owner"});
        assertTrue(notices.get(owner).getFirst().contains("Player declined"));
        c.onCommand(player,null,"plots",new String[]{"decline","Owner"});assertEquals(1,notices.get(owner).size());
        store.invite("other",owner,id,"STOCK",System.currentTimeMillis());
        c.onCommand(player,null,"plots",new String[]{"accept","Owner"});
    ...
    @Test void memberReceivesRoleChangesAndRemoval()throws Exception {
        ...
        store.acceptInvitation(store.invite("one",id,member,"STOCK",now).token(),member,now);
        var c=commands(true,true,()->{},()->{});
        c.onCommand(player,null,"plots",new String[]{"role","one","Member","build"});
        assertTrue(notices.get(member).getFirst().contains("now build"));
        c.onCommand(player,null,"plots",new String[]{"invite","one","Member","both"});
        assertTrue(notices.get(member).getLast().contains("build and stock"));
        c.onCommand(player,null,"plots",new String[]{"remove","one","Member"});
    ...
    @Test void plotListPagesAreBoundedAndRejectInvalidPages()throws Exception{
        ...
        for(int page=1;page<=3;page++){
            lines.clear();c.onCommand(viewer,null,"plots",new String[]{"list",""+page});
        ...
        for(String invalid:List.of("0","4","-1","abc","999999999999999999")){
            lines.clear();c.onCommand(viewer,null,"plots",new String[]{"list",invalid});assertEquals(1,lines.size());
